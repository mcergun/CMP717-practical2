...
%                                      Default: 10^-7
%                * 'useLessAtoms'    : NOT YET IMPLEMENTED
%                                      Use for several iterations less than all the atoms. 
%                                      This is a vector, with numbers for each iteration
%                                      If it is shorter, for later iterations all patches are used
%                                      A number 0.1 means about 0.1 of the patches are used
...
for itr = 1 : param.nIterations
	tic;
	if isfield(param , 'useLessAtoms') && length(param.useLessAtoms) >= itr
		takePatchesInds = unique(round(linspace(1 , size(trainPatches,2) , param.useLessAtoms(itr) * size(trainPatches,2))));
	else
		takePatchesInds = [1 : size(trainPatches,2)];
	end
	currTrainPatches = trainPatches(: , takePatchesInds);
	usingAllPatches = size(currTrainPatches,2) == size(trainPatches , 2);
	
	% Do sparse coding - we need the reconstructed patches for the residual
	[currConstructedPatches , currCoeffs] = Batch_OMP(currDict , currTrainPatches , param);
	
	% Compute average cardinality or representation error, and print information
	if isfield(param , 'errorGoal')
		meanCard = full(sum(abs(currCoeffs(:)) > param.meaningfulCoeff) / length(takePatchesInds));
		disp(['      Iteration ',num2str(itr),' ==> Average Cardinality ',num2str(meanCard)]);
	end
	
	if isfield(param , 'maxAtoms')
		resids = currTrainPatches - currConstructedPatches;
	...
	if isfield(param , 'truePatches')
		signalErrors = param.truePatches(: , takePatchesInds) - currConstructedPatches;
		meanSignalsError = mean(sum(signalErrors.^2 , 1));
	...
	if isfield(param , 'imageData') && usingAllPatches
	...
	switch param.method
		case 'MOD'
			...
			newDict = (currTrainPatches * currCoeffs') / (currCoeffs * currCoeffs' + (eye(nAtoms) * 10^-7));
		...
		case 'KSVD'
			...
			for atomInd = 1 : nAtoms
				...
				resids = currTrainPatches(: , currPatchsInds) - newDict * currPatchsCoeffs;
	...
	resids = currTrainPatches - newDict * currCoeffs;
	...
	for cAtom = 1 : nAtoms
		...
		if ((maxIPwithOtherAtom > param.maxIPforAtoms) || (numTimesUsed <= param.minTimesUsed))
			...
			worstSigInd = find(max(sigRepErr)==sigRepErr,1); 
            worstSigInd = worstSigInd(1);
			newAtom = currTrainPatches(: , worstSigInd);
	...
	if (param.showDictionary > 0) && (mod(itr , param.showDictionary) == 0)
		if itr==1, figure; end; % Not open new figure every time
